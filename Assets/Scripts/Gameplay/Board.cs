...
namespace Tetris
{
    public class Board : MonoBehaviour
    {
        ...
        public void AddGarbageLines(int linesToAdd)
        {
            ...
            // 3. Resolve active piece overlap
            if (activePiece != null)
            {
                // Push piece up until it doesn't overlap
                while (!IsValidPosition(activePiece.cells, activePiece.position))
                {
                    activePiece.ShiftUp(1);
                    
                    // If it pushes above board, trigger game over
                    if (activePiece.position.y >= bounds.yMax)
                    {
                        if (EventManager.Instance != null) EventManager.Instance.TriggerGameOver();
                        GameOver();
                        return;
                    }
                }
